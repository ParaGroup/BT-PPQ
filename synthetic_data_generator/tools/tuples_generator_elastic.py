...
#		   In case of a no-bursty generation, the inter-arrival times are generated using an
#		   exponential distribution with rate lambda. Instead, in case of a certain burstiness,
#		   the inter-arrival times are obtained using a MAP distribution with rate lambda and
#		   fitted with the target index of dispersion passed as a command line argument.
#		   The rate lambda is kept fixed throughout a phase interval. Instead, between distinct
#		   phases, the average rate to be used is dynamically changed following a lognormal random
#		   walk model (the same used for stock prices).
# 
# Date:	   December 2016
...
phase_len = 3000000 # length in usec of a phase interval
volatility = 0.4 # volatility parameter of the random walk model
...
def main():
	...
	for opt, arg in opts:
		if opt in ('-n'):
			...
			rate = float(arg) # starting rate per second in no. of tuples
		elif opt in ('-i'):
	...
	dataset[:,:-1] = m
	# starting average inter-arrival time in timeunits (usec)
	TA = 1000000.0/rate
	...
	for i in range(n):
		...
		# check if we need to change the phase
		if(base > phase_end):
			phase_end = phase_end + phase_len
			# X is defined as log(TA_{k}/TA_{k-1})
			...
			if(TA > 300):
				TA = 300
			# fitting another MAP distribution if needed
			if(disp_index > 0):
				distribution = MAP_Distribution(TA, disp_index);
			#sys.stdout.write("New Phase with rate " + str(1000000/TA) + "\n")
